$(function () {
    ...
    //     location.assign('checkout.html?total='+$('.money').text());
    // });
    shop.api.fetchCart(function (response) {
        ...
        if(response.data.length>0){
            for(var i =0;i<response.data.length;i++){
                ...
                $(".cart").append("<div class='cart-goods'><div><input type='checkbox' class='chkbox' /><input type='hidden' class='goods_id' value='"+obj.goods_id+"'></div><div><img src='" + obj.goods_thumb + "'></div>" +
                    "<div><p>" + obj.goods_name + "</p><span class='good-price'>" + obj.goods_price +"</span>'<span class='subtotal'>" +
                    obj.subtotal + "</span><button onclick='minusGoods(this)'>-</button>"
                    + "<input class='goods_number' type='text' value='"+obj.goods_number+"'>"
                    + "<button onclick='plusGoods(this)'>+</button><span class='delete' onclick='deleteGoods(this)'>"+"删除" +"</span></div></div>");
            }
            showSum();
        }
    })
...
function showSum() {
    var subtotal = $('.cart>div');
    //找到每行中的小计，累加起来就是总和
    var sum = 0;
    for (var i = 0; i < subtotal.length; i++) {
        var sub = subtotal[i];
        //判断一下当前行的选中框是否选中，如果选中则计算到总价中
        console.log($(sub).children("div:first"));
        console.log($(sub).children("div:eq(0)").children('input'));
        console.log($(sub).children("div:eq(0)").children('input').is(':checked'));
        if ($(sub).children("div:eq(0)").children('input').is(':checked')) {
            console.log($(sub).children('div:eq(2)').children('span:eq(1)').text());
            sum += parseInt($(sub).children('div:eq(2)').children('span:eq(1)').text());
        }
    }
    $('.money').text(sum);
}
...
$('body').click(function(event){
    ...
    if (event.target.className === "subtotal") {
        console.log('++');
        //修改数量
        var oNumber = $(event.target).next().next();
        var number = oNumber.val();
        oNumber.val(++number);
        // console.log(number);
        //小计调整
        var price = parseInt($(event.target).prev().text());
        console.log(price);
...
function updateCart(obj, num) {
    ...
    //请求ajax
    shop.api.updateCart(goods_id, goods_number_value, function(){
    });
    //显示总价
    showSum();
}